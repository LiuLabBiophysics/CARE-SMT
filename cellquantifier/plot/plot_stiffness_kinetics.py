...
def plot_stiffness_kinetics(
    ...
    if not df.empty:
        ...
        # """
        if '0.2kPa' in exp_labels:
            dfp1 = df[ df['exp_label']=='0.2kPa' ].drop_duplicates('frame')
            ...
            dfp1.round(3).to_csv('/home/linhua/Desktop/0.2kPa_data.csv', index=False)
        else:
            dfp1 = pd.DataFrame([], columns=['frame', 'avg_foci_num', 'exp_label'])


        if '2kPa' in exp_labels:
            dfp2 = df[ df['exp_label']=='2kPa' ].drop_duplicates('frame')
            dfp2['avg_foci_num'] = dfp2['avg_foci_num'] / dfp2['avg_foci_num'].max()
            dfp2 = dfp2[ ['frame', 'avg_foci_num', 'exp_label'] ]
            dfp2 = dfp2.sort_values(by='frame')
            dfp2.round(3).to_csv('/home/linhua/Desktop/2kPa_data.csv', index=False)
        else:
            dfp2 = pd.DataFrame([], columns=['frame', 'avg_foci_num', 'exp_label'])


        if '12kPa' in exp_labels:
            dfp3 = df[ df['exp_label']=='12kPa' ].drop_duplicates('frame')
            dfp3['avg_foci_num'] = dfp3['avg_foci_num'] / dfp3['avg_foci_num'].max()
            dfp3 = dfp3[ ['frame', 'avg_foci_num', 'exp_label'] ]
            dfp3 = dfp3.sort_values(by='frame')
            dfp3.round(3).to_csv('/home/linhua/Desktop/12kPa_data.csv', index=False)
        else:
            dfp3 = pd.DataFrame([], columns=['frame', 'avg_foci_num', 'exp_label'])


        if 'glass' in exp_labels:
            dfp4 = df[ df['exp_label']=='glass' ].drop_duplicates('frame')
            ...
            dfp4.round(3).to_csv('/home/linhua/Desktop/glass_data.csv', index=False)
        else:
            dfp4 = pd.DataFrame([], columns=['frame', 'avg_foci_num', 'exp_label'])
