...
class Pipeline3():
	...
	def anim_traj(self):
		...
		if 'Sort dist_to_boundary' in phys_df:
			dist2bdr_min = self.config.DICT['Sort dist_to_boundary'][0]
			dist2bdr_max = self.config.DICT['Sort dist_to_boundary'][1]
			phys_df = phys_df[ (phys_df['dist_to_boundary']>dist2bdr_min) & \
								(phys_df['dist_to_boundary']>dist2bdr_max)]
		...
		anim_tif = anim_traj(phys_df, frames,
					pixel_size=self.config.PIXEL_SIZE,
					# cb_min=D_min - 0.1*D_range,
					# cb_max=D_max + 0.1*D_range,
