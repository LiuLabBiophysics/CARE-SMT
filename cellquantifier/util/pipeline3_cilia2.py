...
from ..plot.plotutil._add_mean_msd2 import *
from ..publish._fig_quick_msd import *
...
class Pipeline3():
	...
	def check_detect_fit(self):
		...
		for ind in check_frame_ind:
			...
			# psf_df, fit_plt_array = fit_psf(frames_deno[ind],
			#             blobs_df,
			#             diagnostic=True,
			#             pltshow=True,
	...
	def filt_track(self):
		...
		if check_search_range:
			...
			for search_range in param_list:
				print("\n")
				print("Optimizing (%d/%d): %s" % (ind, tot, search_range))
				ind = ind + 1
		...
		if check_traj_len_thres:
			...
			for traj_len_thres in param_list:
				print("\n")
				print("Optimizing (%d/%d): %s" % (ind, tot, traj_len_thres))
				ind = ind + 1
			...
			blobs_df['pixel_size'] = self.config.PIXEL_SIZE
			blobs_df['frame_rate'] = self.config.FRAME_RATE
	...
	def merge_plot(self):
		...
		if len(merged_files) == 1:
			...
			if len(phys_files) > 1:
				...
				phys_df = merge_physdfs(phys_files, format='general')
